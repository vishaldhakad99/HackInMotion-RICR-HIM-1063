...
const Analytics = () => {
  ...
  const fetchAllAnalytics = async () => {
    try {
      ...
      const [ovRes, catRes, stRes, deptRes, hotRes, resTimeRes] = await Promise.all([
      ...
      if (resTimeRes.success) setResolutionTimeData(resTimeRes.data || []);
    } catch (err) {
      ...
      <div className="flex-1 flex w-full">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 max-w-full overflow-hidden">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-extrabold text-slate-900">City Analytics & Performance</h1>
              <p className="text-xs text-slate-500 mt-1">
                Data-driven insights on complaint volume, category resolution rates, and departmental turnaround.
              </p>
            </div>
          ...
          {loading ? (
            ...
            <ErrorMessage message={error} onRetry={fetchAllAnalytics} />
          ) : (
            <>
              {/* Metric Cards */}
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-2">
                  <span className="text-xs font-semibold text-slate-500">Total City Reports</span>
                  <p className="text-3xl font-black text-slate-900">{overview?.totalIssues || 0}</p>
                </div>

                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-2">
                  <span className="text-xs font-semibold text-slate-500">Resolution Rate</span>
                  <p className="text-3xl font-black text-emerald-600">{overview?.resolutionRate || 0}%</p>
                </div>

                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-2">
                  <span className="text-xs font-semibold text-slate-500">Avg Turnaround (Hours)</span>
                  <p className="text-3xl font-black text-blue-600">{overview?.avgResolutionTimeHours || 0} hrs</p>
                </div>

                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-2">
                  <span className="text-xs font-semibold text-slate-500">Total Upvotes</span>
                  <p className="text-3xl font-black text-indigo-600">{overview?.totalUpvotes || 0}</p>
                </div>
              </div>
                  ...
                  <div className="h-72 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={categoriesData} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                        ...
                        <Tooltip />
                        <Bar dataKey="totalIssues" fill="#3b82f6" name="Total Issues" radius={[4, 4, 0, 0]} />
                        <Bar dataKey="resolvedIssues" fill="#10b981" name="Resolved Issues" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-4">
                <h3 className="text-sm font-bold text-slate-900">Department Performance Breakdown</h3>
                    ...
                    <tbody className="divide-y divide-slate-100 font-medium">
                      {deptData.map((dept) => (
                        <tr key={dept.departmentId} className="hover:bg-slate-50 transition">
                          <td className="py-3 px-4 font-bold text-slate-900">{dept.name} ({dept.code})</td>
                          <td className="py-3 px-4 font-bold">{dept.totalIssues}</td>
                          <td className="py-3 px-4 text-emerald-600 font-bold">{dept.resolvedIssues}</td>
