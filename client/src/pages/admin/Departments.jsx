        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 max-w-full overflow-hidden">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Manage departmental routing queues, contacts, and active status.</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {departments.map((dept) => (
                <div key={dept._id} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1 line-clamp-2 leading-relaxed">{dept.description}</p>
                  </div>

                  <div className="pt-3 border-t border-slate-100 space-y-2 text-xs text-slate-600">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400">Department Head:</span>
                      <span className="font-bold text-slate-800">{dept.headName || "Unassigned"}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400">Contact Email:</span>
                      <span className="font-medium text-slate-700">{dept.contactEmail || "contact@civic.gov.in"}</span>
                    </div>
                  </div>

                  <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 grid grid-cols-3 gap-2 text-center text-[11px]">
                    <div>
                      <p className="font-bold text-slate-900">{dept.stats?.totalIssues || 0}</p>
                      <p className="text-slate-400 text-[10px]">Total</p>
                    </div>
                    <div>
                      <p className="font-bold text-amber-600">{dept.stats?.openIssues || 0}</p>
                      <p className="text-slate-400 text-[10px]">Pending</p>
                    </div>
                    <div>
                      <p className="font-bold text-emerald-600">{dept.stats?.resolvedIssues || 0}</p>
